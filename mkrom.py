...
if __name__ == "__main__":
    output = bytearray(2**BITS * DIGITS * len(Mode))
    for value in range(-2**(BITS-1), 2**(BITS-1)):
        for mode in Mode:
            ...
            for idx, ch in enumerate(text):
                addr = value % (2**BITS)
                addr |= (idx + mode.value*DIGITS) << BITS
                output[addr] = shuffle(FONT[ch], "pgfabcde")
